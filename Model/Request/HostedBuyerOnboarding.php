...
class HostedBuyerOnboarding extends CommonRequest implements RequestInterface
{
    protected $curl;
    ...
    /**
     * Creates a hosted buyer onboarding session.
     *
    ...
    public function request($params = null)
    {
        $resultJson = $this->sendRequestWithParams(
            'post',
            $this->urlBuilder->getHostedBuyerOnboardingUrl(),
            json_encode($params)
        );

        if (!$resultJson) {
            throw new LocalizedException(__('Failed to create buyer onboarding session'));
        }
