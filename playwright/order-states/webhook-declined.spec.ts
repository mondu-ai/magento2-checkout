...
import { sendWebhook, buildOrderDeclinedPayload } from '../helpers/webhook'
import { checkMonduOrderState, getMonduOrder } from '../helpers/api'
import { loginToAdmin, openOrderByMagentoId } from '../helpers/admin'

test('Webhook order/declined → Magento order is canceled', async ({ page }) => {
  ...
  // Send order/declined webhook
  const payload = buildOrderDeclinedPayload(orderUuid!, externalRefId, 'risk_policy')
  const webhookResponse = await sendWebhook(apiContext, 'order/declined', payload)
  expect(webhookResponse.status()).toBe(200)

  // Poll until Magento order status changes to canceled
  await expect(async () => {
    await page.reload()
