...
test('Create admin order with Mondu Installment (legal_form + installments + iban + account_holder)', async ({ page }) => {
  ...
  await expect(page.locator('[data-mondu-field="mondu_iban"]')).toBeVisible()
  await expect(page.locator('[data-mondu-field="mondu_account_holder"]')).toBeVisible()
  // net_term should be hidden for installment
  ...
  // Fill IBAN via typing to test sanitization
  ...
  if (successOrError === 'error') {
    ...
    // If minimum amount error — expected for low-value products with installments
  ...
  console.log(`Installment admin order created: #${orderNumber}`)
  ...
  if (monduUuid) {
    ...
    expect(monduOrder.payment_method).toBe('installment')
  }
